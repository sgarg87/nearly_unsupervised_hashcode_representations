...
class HashFunctionRepresentations:
    ...
    def __init__(
        ...
        self.ept_obj = ept.EmbedPathTuples(is_word_only=(not is_edge_label))

        self.is_cluster_high_weightage = is_cluster_high_weightage
        assert isinstance(cluster_sample_weights, (int, long))
    ...
    def compute_score_for_reference_subset_partition(self,
        ...
        curr_score = Hz + self.nonredundancy_score_beta*Hz_cond_C
    ...
    def compare_array_wrt_array_of_arrays(self, array_for_comparison, arrays):
        ...
        for curr_idx in xrange(num_arrays_for_comparison):
    ...
    def get_all_combinations(self, subset_size, superset=None):
        ...
        num_combinations = 0
        for curr_combination_size in xrange(1, alpha+1):
    ...
    def optimize_rmm_hash_for_subsets(
        ...
        if self.is_opt_reg_rmm:
            ...
            for curr_reg_val_idx in xrange(reg_values.size):
    ...
    def compute_scores_for_optimize_reference_subsets_wrapper(
        ...
        for curr_combination_idx in xrange(num_combinations):
    ...
    def subselect_bits(self, C, max_num_bits):
        ...
        if C is None:
            ...
            if len(C.shape) == 2:
                ...
                if num_bits > max_num_bits:
                    rnd_subset_bits = self.npr_sample_hash_bits_obj.choice(num_bits, max_num_bits, replace=False)
                    C = C[:, rnd_subset_bits]
    ...
    def select_high_entropy_cluster(self,
        ...
        if is_return_neighboring_cluster_data_indices:
            if is_return_cluster_data_indices:
                return subset_idx, subset_size, data_idx_from_cluster, data_idx_from_neighbor_clusters
            else:
                return subset_idx, subset_size, data_idx_from_neighbor_clusters
        else:
            if is_return_cluster_data_indices:
    ...
    def infer_labels_for_unlabeled_examples_in_clusters(self, C_org, x, max_num_bits):
        ...
        for curr_trial_idx in xrange(num_trials):
    ...
    def compute_subset_size_per_cluster_size(self, subset_size, cluster_size):
        subset_size = max(subset_size,
                          int(math.log(cluster_size, self.alpha_compute_per_cluster_size__log_scale)**2)
                    )
        return subset_size
    ...
    def find_neighboring_clusters_including_the_cluster(self, Cu, cluster):
        ...
        if len(Cu.shape) == 1:
            ...
            for curr_idx in xrange(num_clusters):
            ...
            neighbor_hamming_distance_rad = int(round(neighbor_hamming_distance_max/10.0))
    ...
    def sample_from_train_set(self,
                              path_tuples_all, x, C, sample_counts=None):
    ...
    def optimize_kernel_hash_func(self,
                                  path_tuples_all,
                                  curr_subset_path_tuples_kernel_references,
                                  C,
                                  x,
    ...
    def format_path_tuple_as_text(self, curr_element_in_ref_set):
        curr_text = ' '.join([curr_pos_tuple[0] for curr_pos_tuple in curr_element_in_ref_set['path_tuple']])
        curr_text = '\'{}\''.format(curr_text)
        return curr_text

    def get_sample_weights_normalized_from_counts(self,
                                                  path_tuples_all_counts,
                                                  path_tuples_all_vocab_counts,
                                                  idx=None):
    ...
    def optimize_reference_subsets_greedy_stochastic(
        ...
        if y is not None:
            assert x is not None
            assert np.where(x == 1)[0].size == y.size
        ...
        if self.hash_func == 'RNeural':
            if self.is_images:
                ...
                # path_tuples_all_embedding = self.data_operations_obj.embed_path_tuples(path_tuples_all)
                path_tuples_all_embedding = self.ept_obj.embed_path_tuples(path_tuples_all)
        ...
        while curr_set_idx < (num_hash_functions-1):
            ...
            if (not self.is_alpha_sampling_weighted) or (curr_set_idx < 15):
                ...
                for curr_idx in xrange(num_subset_sizes):
            ...
            C_subsel_bits = self.subselect_bits(C, num_bits_for_sel_high_entropy_cluster)

            if (C_subsel_bits is not None) and (len(C_subsel_bits.shape) == 2) and (C_subsel_bits.shape[1] > 1):
                subset_size = int(subset_size/math.log(C_subsel_bits.shape[1], 2))
            ...
            if curr_set_idx == 0:
                ...
                if self.is_inductive_ssh:
                    ...
                    if not self.is_joint_inductive_transductive_ssh:
                        ...
                        # test and training data
                        idx_12 = np.where((x == 1) | (x == 2))[0]
                        curr_subset12, subset_size, data_idx_from_cluster,\
                        data_idx_from_neighbor_clusters_and_the_cluster = self.select_high_entropy_cluster(
                        ...
                        # test and training data
                        curr_subset12, subset_size, data_idx_from_cluster, data_idx_from_neighbor_clusters_and_the_cluster\
            ...
            if data_idx_from_neighbor_clusters_and_the_cluster is not None:
                print 'data_idx_from_neighbor_clusters_and_the_cluster.size',\
                    data_idx_from_neighbor_clusters_and_the_cluster.size
            else:
